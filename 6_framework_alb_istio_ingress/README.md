ansible/roles/eks/app/cloudformation/alb/irsa.yaml
```
AWSTemplateFormatVersion: "2010-09-09"
Description: "Kubeflow IRSA for ALB"
...
Parameters:
  ...
  OIDC:
    ...
    Default: oidc.eks.us-west-2.amazonaws.com/id/TODO
  Namespace:
    Type: String
    Description: EKS namespace of alb controller
    Default: kube-system
  SARole:
    Type: String
    Description: EKS namespace service account role
...
Resources:
  AWSLBCRole:
    ...
    Properties:
      RoleName: !Ref SARole
      Policies:
        - PolicyName: AWSLBCRole_role_policy
          PolicyDocument:
            Version: '2012-10-17'
            Statement:
              - Effect: Allow
      ...
      AssumeRolePolicyDocument:
        Fn::Sub:
              ...
              "Statement": [
                {
                  "Effect": "Allow",
                  "Principal": {
                    "Federated": "${principal_id}"
                  },
                  "Action": "sts:AssumeRoleWithWebIdentity",
                  "Condition": {
                    "StringEquals": {
                      "${clusterid}:sub": "${serviceaccount}",
          ...
          -
            clusterid: !Ref OIDC
            principal_id: !Join [ "", ["arn:aws:iam::", !Ref AccountID, ":oidc-provider/", !Ref OIDC]]
            serviceaccount: !Join ["", ["system:serviceaccount:", !Ref Namespace, ":", !Ref SARole]]

```

ansible/roles/eks/app/templates/albc.yaml.j2
```
...
- name: IRSA for ALB controller
  cloudformation:
    ...
    template: "roles/eks/app/cloudformation/alb/irsa.yaml"
...
- name: prepare aws lbc
  template:
    src: albc.yaml.j2
    dest: roles/eks/app/albc.yaml
  vars:
...
append to the end of ansible/roles/eks/tasks/main.yaml
```
- name: setup ALBC
  import_tasks: alb.yaml
```

Run eks role

Now let's update kubeflow role

ansible/roles/kubeflow/app/other/ingress.yaml
```
...
ansible/roles/kubeflow/tasks/alb.yaml

Do it on your own:

One option is to wrap it with ansible and include in main.yaml. It is good as POC
```
kubectl apply -f app/noble/other/ingress.yaml

kubectl get svc -A

kubectl get ingress -A

kubectl logs -f aws-load-balancer-controller-749b54998c-c64ws -n kubesystem
```
